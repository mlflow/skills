...
- **Token usage explains latency and cost.** Look for token usage in trace metadata (e.g., `mlflow.tokenUsage`) or span attributes (e.g., `mlflow.chat.usage`). Not all clients set these — check the raw `attributes` dict for equivalent fields. Spikes in input tokens may indicate prompt injection or overly large context.
