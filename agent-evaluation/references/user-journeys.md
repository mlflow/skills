...
1. **Establish baseline metrics**
   ...
   ```
   Record key metrics from previous run
...
3. **Compare metrics**
   ...
   - Compare pass rates for each scorer
...
5. **Analyze failing traces**
   - Review traces that failed in current but passed in baseline
   - Look for:
...
2. **Identify lowest-performing metrics**
...
6. **Document successful optimizations**
   Keep notes on what worked and why
...
| Tool | Purpose |
|------|---------|
| `run_evaluation_template.py` | Run evaluation |
| `analyze_results.py` | Result analysis |
| `list_datasets.py` | Dataset discovery |
| `validate_environment.py` | Environment check |
